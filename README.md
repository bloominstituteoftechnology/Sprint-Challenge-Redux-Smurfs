...
Actions are functions that are sent to reducers through a method called dispatch. Whether it be an API call or an actionable object, it will notify the reducer if and when to fire off. When the reducer fires, it changes the state of the application, and communicates this with the store. Store is where the state of the entire application is being held.
...
Redux-thunk is a type of middleWare that will allow action creators to return asynchronous or conditional functions (or any functions). Without thunk, we are not able to perform functions inside of the action creators. Actions return objects without it in redux.
