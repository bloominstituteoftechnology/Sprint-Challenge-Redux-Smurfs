...
export const addSmurf = e => async dispatch => {
  ...
  const { name, age, height } = e.target.elements;

  dispatch({ type: ADDING_SMURF });

  try {
    const { data: smurfs } = await post("http://localhost:3333/smurfs", {
      name: name.value,
      age: age.value,
      height: height.value
    });
...
export const getSmurfs = () => async dispatch => {
  ...
  try {
    const { data: smurfs } = await get("http://localhost:3333/smurfs");
...
export const deleteSmurf = id => async dispatch => {
  ...
  try {
    const { data: smurfs } = await _delete(
